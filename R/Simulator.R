...
for(yy in 1:length(Years)){
            ...
            if(yy==1){
              for(aa in 1:NAges){
                ...
                N[ yy, 1, rr, ss, aa] <- RDistSP[which(RDistSP$StockID==Stocks[ss] & RDistSP$Age==Ages[aa] & RDistSP$Period==1), 
                                               which(names(RDistSP)==paste("P", rr, sep="_"))]  *  Cohort[yy,ss,aa]
              } # end age loop
            } else { # end yy == 1
    ...
    if(pp==1){
      ...
      for(ff in ff_ISBM){
        ...
        for(ss in 1:NS){
            ...
            Catch[yy,,region, gear, ss, ] <- (cbind(Tao) %*% N[yy,1,region,ss, ]) * h[yy,,region,gear,ss, ]
            # values are tiny!
  ...
  for(ss in 1:NS){
    ...
    if(as.character(StocksInfo$Hatchery[which(StocksInfo$StockID==Stocks[ss])])=="N"){
      ...
      Recruits[yy, ss] <-  Spawners[yy, ss] * exp( a*(1- Spawners[yy, ss]/B))
